...
#' Get and parse the changes logged by the afterChange hook
#'
#' @param input The input list from the server function
#' @param hotwidget_data The original data set
#'
#' @return A list with the data set and a boolean indicating if an error occured.
get_afterchange <- function(input, hotwidget_data) {
  tryCatch(
    {
      data <-
        map2(
          .x = input$hotwidget_afterchange$changes,
          .y = input$hotwidget_afterchange$physical_key,
          \(.x, .y) {
            as_tibble(
              map(.x, \(.x) ifelse(is.null(.x),NA,.x)),
              .name_repair = ~c("client_row", "col", "old", "new")) |>
